...
> **Última actualización:** Abril 2026 — Arquitectura de entrega confiable (`marcar_enviado.php`)  
...
### Flujo de entrega confiable (2 pasos)

```
[ERP / Base de datos]
        ↑ consulta SQL
[check_all.php]  ← GET cada 1 min (X-WSP-Token)
        ↓ retorna alertas SIN marcar en BD
[PitayaBot — scheduler.js]
        ↓ sendMessage() por cada destinatario
        ↓ ¿Al menos 1 éxito?
        ├── SÍ → POST marcar_enviado.php → INSERT en alertas_wsp_estado
        └── NO → alerta NO se marca → se reintenta el próximo minuto
[Colaboradores — WhatsApp]
```

> **Principio clave:** La alerta solo se registra en `alertas_wsp_estado` **después** de confirmar
> que al menos un mensaje fue entregado exitosamente. Si el bot falla (error de red, WhatsApp
> caído, etc.), la alerta no queda marcada y se reintenta automáticamente en el siguiente ciclo.
...
| **Fallo de envío** | La alerta NO se marca → se reintenta automáticamente |
| **Horario** | 24/7, sin restricción |
| **Registro de entrega** | El bot confirma vía `marcar_enviado.php` tras envío exitoso |
...
├── check_all.php                ← Orquestador (endpoint que llama PitayaBot cada 1 min)
├── alerta_conexion_pc.php       ← Alerta: PC offline ≥ 60 min
├── alerta_anulacion_web.php     ← Alerta: Anulación web pendiente del día
├── marcar_enviado.php           ← Confirma entrega exitosa (llamado por el bot)
...
> **Sobre `enviado_at`:** El campo tiene `DEFAULT CURRENT_TIMESTAMP`, por lo que se llena
> automáticamente al hacer el `INSERT`. Como el INSERT solo ocurre después de confirmar entrega
> exitosa (vía `marcar_enviado.php`), `enviado_at` refleja el momento real del envío.
...
6. **Retornar** el array con formato estándar **SIN hacer INSERT** — el registro lo hace el bot vía `marcar_enviado.php`:

```php
return ['alertas' => $alertas];
```

> ⚠️ **El archivo de alerta NO debe hacer `INSERT INTO alertas_wsp_estado`.**
> El INSERT lo hace `marcar_enviado.php` solo después de que el bot confirme entrega exitosa.
> Esto garantiza que si el bot falla al enviar, la alerta se reintente el próximo ciclo.
...
[
    'alertas' => [
        [
            ...
            'key_unica'     => 'identificador-evento', // lo que se guardará en alertas_wsp_estado
            'mensaje'       => "🔔 *Título*\n...",     // texto WhatsApp con emojis y *bold*
            'destinatarios' => ['88001234', '88005678'], // sin código de país (se agrega 505 en el bot)
            'datos_json'    => [                        // contexto para guardar en la BD
                'campo1' => $valor1,
                'campo2' => $valor2,
            ],
        ],
...
> **`datos_json`** es enviado por el bot a `marcar_enviado.php` para que quede registrado
> en `alertas_wsp_estado.datos_json`. Permite auditar el contexto exacto del evento al
> momento de la entrega.

---

## 5. `marcar_enviado.php` — Confirmación de entrega

**Endpoint:** `POST /api/alertas/marcar_enviado.php`  
**Header:** `X-WSP-Token: <token>`  
**Body JSON:**
```json
{
    "tipo_alerta": "anulacion_web",
    "key_unica":   "9379",
    "datos_json":  { "CodPedido": 128787, "nombre_sucursal": "Altamira" }
}
```

**Respuesta exitosa:**
```json
{ "success": true, "registrado": true, "tipo": "anulacion_web", "key": "9379" }
```

| Campo | Descripción |
|---|---|
| `registrado: true` | INSERT fue exitoso (nueva alerta registrada) |
| `registrado: false` | Key ya existía (INSERT IGNORE no actuó) — no es un error |

> Llamado únicamente desde `scheduler.js → marcarAlertaEnviada()` tras `sendMessage()` exitoso.

---

## 6. Query de Destinatarios (Reutilizable)
...
## 7. `check_all.php` — Cómo agregar una nueva alerta
...
## 8. `scheduler.js` — Sin cambios necesarios para nuevas alertas

El cron de alertas ya está registrado y maneja el flujo de confirmación automáticamente:

```js
// Cada 1 minuto: detecta, envía y confirma
cron.schedule('* * * * *', () => {
    ejecutarAlertas(clienteWA);
}, { timezone: TZ });
```

**No es necesario modificar `scheduler.js`** al agregar nuevas alertas. Solo se modifica `check_all.php` y el nuevo archivo PHP. El bot llamará a `marcarAlertaEnviada()` automáticamente para cualquier alerta que incluya `tipo`, `key_unica` y `datos_json`.

---

## 9. `gestion_permisos.php` — Sin cambios necesarios
...
## 10. Plantilla Completa para Nueva Alerta
 ...
 *
 * IMPORTANTE: Este archivo NO registra en alertas_wsp_estado.
...
try {
    ...
    foreach ($registros as $r) {
        ...
        $alertas[] = [
            ...
            'datos_json'    => [          // contexto que quedará en la BD
                'campo1' => $r['campo1'],
                'campo2' => $r['campo2'],
            ],
        ];
...
## 11. Guía de Formato de Mensajes WhatsApp
...
- **Entrega confiable:** Los archivos PHP de alerta **no hacen INSERT**. El bot llama a `marcar_enviado.php` solo tras `sendMessage()` exitoso. Si el bot falla, la alerta se reintenta el próximo minuto automáticamente.
- **`INSERT IGNORE` en `marcar_enviado.php`:** Previene errores de duplicate key en condiciones de carrera (2 ciclos simultáneos del bot). Si la key ya existe, retorna `registrado: false` sin error.
...
- **Sin `activo` en `alertas_wsp_estado`:** El UNIQUE key es la única barrera. No se necesita columna de estado activo/inactivo — el diseño de `key_unica` maneja los reinicios naturales.
...
- **`datos_json` en array de alerta:** Siempre incluirlo en el array retornado — el bot lo reenvía a `marcar_enviado.php` para auditoría del contexto al momento del envío.
